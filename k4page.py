...
class K4Page:
    ...
    def generate_pdf(self, destination_folder):
        ...
        def generate_section(pdf, section, maxlines, ystart, yoffset, xp, sum_yoffset):
            for (y, fields) in enumerate(section.lines):
                for (x, field) in enumerate(fields):
                    ys = ystart - yoffset * y
                    if field:
                        pdf.drawString(x=xp[x], y=ys, text=field[:9])
            for (x, field) in enumerate(section.sums):
                if field:
                    pdf.drawString(x=xp[x + 2], y=ystart - yoffset * maxlines - sum_yoffset, text=field[:9])
        ...
        def generate_page_1_overlay():
            ...
            if self._section_a and self._section_a.lines:
                generate_section(pdf, self._section_a, 9, 588, 24, [58, 122, 217, 302, 388, 475], 10)
            pdf.save()
            data.seek(0)
            return data
        ...
        def generate_page_2_overlay():
            ...
            if self._section_c and self._section_c.lines:
                generate_section(pdf, self._section_c, 7, 720, 24, [58, 122, 217, 302, 388, 475], 10)
            if self._section_d and self._section_d.lines:
                generate_section(pdf, self._section_d, 7, 360, 24, [58, 122, 217, 302, 388, 475], 10)
            pdf.save()
            data.seek(0)
            return data
