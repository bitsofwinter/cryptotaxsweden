...
class Coin:
    ...
    def buy(self, amount:float, price:float):
        ...
        self._cost_basis = (self._cost_basis * self._amount + price) / new_amount
        self._amount = new_amount

    def sell(self, amount:float, price:float) -> TaxEvent:
        amount_left = self._amount - amount
        if amount_left < -1e-9:
            raise Exception(f"Not enough coins available for {self._symbol}, {self._amount} < {amount}.")
        if amount_left < 0.0:
            amount_left = 0.0

        tax_event = TaxEvent(amount, self._symbol, price, self._cost_basis * amount)

        self._amount = amount_left

        return tax_event


def compute_tax(trades, from_date, to_date, native_currency='SEK', exclude_groups=[]):
    tax_events = []
    coins = {}
...
def convert_to_integer_amounts(tax_events, precision_loss_tolerance=0.1):
    ...
    # Convert tax events to integer amounts
    new_events = []
...
def generate_k4_pages(year, personal_details, tax_events, stock_tax_events=None):
    def generate_section(events):
        ...
        for event in events:
            ...
            lines.append(line)
        sums = [str(sum) for sum in num_sums if sum > 0]
